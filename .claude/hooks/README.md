...
| ↳ `guard-instructions` | | правка `CLAUDE.md`, `AGENTS.md`, `.mcp.json`, `.claude/rules\|skills\|agents\|commands\|output-styles\|plugins/` — **вопрос** (кроме полного доверия); `.claude/hooks/` и `settings*.json` — **вопрос всегда**; `docs/ai/` — молча |

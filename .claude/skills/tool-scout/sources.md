...
Порядок = доверие. Выше — надёжнее. Цифры звёзд и размеров — ориентир на дату в заголовке,
заново не проверяются: ссылаться на них как на факт нельзя, это только повод посмотреть.
Каталог не открылся или молчит — так и сказать владельцу, а не пересказывать его описание
и не называть «крупнейшим». Перед установкой всегда: чтение содержимого, `compat-check`, «да».

...
0. Обновление идёт без вопросов владельцу: в начале создать `.claude/starter.lock` с текстом
   `{"mode": "update", "начато": "<дата и время>"}` — пока метка есть (час), хук не спрашивает про
   замену файлов набора, которые владелец не менял. В конце (шаг 6) метку удалить.
...
4. Применить: «добавить» и «заменить» — скопировать; «изменён владельцем» — показать разницу,
   ...
   `.gitignore` проекта не заменять никогда — только дописать недостающие строки из `gitignore.starter`.
   Файл уже попал в отпечатки от старой версии — убрать его оттуда при записи нового `starter.json`.
...
6. Удалить `.claude/starter.lock`. Проверить: тесты хуков
   (`python3 .claude/hooks/tests/run_tests.py`, Windows `py -3 …`).
